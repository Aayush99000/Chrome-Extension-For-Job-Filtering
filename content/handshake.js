(() => {
  console.log("🎯 Job Filter AI: Handshake script loaded");
  ...
  async function scanJobs() {
    ...
    console.log("🔍 Scanning Handshake jobs...");

    // Handshake job card selectors
    const jobCards = document.querySelectorAll(
      '[data-hook="search-result"], .style__job-tile, .job-list-item'
    );
    ...
    console.log(
      `✅ Scan complete: ${matchedCount}/${jobCards.length} jobs matched`
    );
    isScanning = false;
  }
  ...
  function extractJobData(card) {
    try {
      const titleEl = card.querySelector(
        '[data-hook="search-result-title"], .style__title, h3'
      );

      const companyEl = card.querySelector(
        '[data-hook="search-result-employer"], .style__employer-name, .employer'
      );

      const locationEl = card.querySelector(
        '[data-hook="search-result-location"], .style__location, .location'
      );
  ...
  function clearFilters() {
    document
      .querySelectorAll(".job-filter-badge, .job-filter-reasons")
